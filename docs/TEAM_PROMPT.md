...
> **Archive.** This is the prompt that was actually sent to the three agents at
> the start of the parallel sprint, kept unchanged as a record of how the work was
> split. It is not current instruction — the code on `main` is. See
> [AGENT_WORKFLOW.md](AGENT_WORKFLOW.md) for what happened when it was run.
